...
//!            + AES‑256‑GCM‑SIV authenticated encryptor (`--features aes`)
...
#[cfg(feature = "keygen")]
mod key; // deterministic key generator (src/key.rs)

#[cfg(feature = "aes")]
mod aes; // AES‑256‑GCM‑SIV encryptor (src/aes.rs)
...
enum Command {
    ...
    /// AES‑256‑GCM‑SIV file encryptor/decryptor
    #[cfg(feature = "aes")]
    Aes(aes::AesArgs),
}
...
fn main() -> Result<()> {
    ...
    if looks_like_sub {
        ...
        match cli.cmd.expect("sub‑command is present") {
            ...
            #[cfg(feature = "aes")]
            Command::Aes(aargs) => aes::run(aargs),
        }
    } else {
