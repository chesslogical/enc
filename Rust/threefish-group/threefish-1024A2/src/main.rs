...
//! • Files are processed **in place**.  If a valid header is found the file
//!   is decrypted; otherwise it is encrypted.  Use `--encrypt` or `--decrypt`
//!   to force a mode.
...
/// Encrypts **or** decrypts FILE in place, deciding from the magic header.
#[derive(Parser, Debug)]
#[command(author, version, about)]
struct Cli {
    /// File to process (encrypted or plain)
    path: PathBuf,

    /// Force encryption, ignoring detected header
    #[arg(long, conflicts_with = "decrypt")]
    encrypt: bool,

    /// Force decryption, ignoring detected header
    #[arg(long, conflicts_with = "encrypt")]
    decrypt: bool,

    /// Suppress “foo → encrypted/decrypted” message
    #[arg(long, short)]
    quiet: bool,
}
