...
const NONCE_SIZE: usize = 16;       // EAX supports 128-bit nonce
const SALT_SIZE: usize = 16;

type KuznyechikEax = Eax<Kuznyechik>;
...
fn encrypt_file(filename: &str, password: &str) {
    ...
    let cipher = KuznyechikEax::new(GenericArray::from_slice(&key));
    ...
    fs::write(&tmp_path, &result).expect("Failed to write file");
...
fn decrypt_file(filename: &str, password: &str) {
    ...
    let tmp_path = format!("{}.tmp", filename);
    fs::write(&tmp_path, &plaintext).expect("Failed to write decrypted file");
    fs::rename(tmp_path, filename).expect("Failed to overwrite file");
