...
pub fn main() !void {
    ...
    if (args.len != 4) {
        std.debug.print("Usage: {s} <enc|dec> <input_file> <output_file>\n", .{args[0]});
    ...
    const key_path = "key.bin";

    if (std.mem.eql(u8, mode, "enc")) {
        // Generate key and save to file
        crypto.random.bytes(&key);
        const key_file = try fs.cwd().createFile(key_path, .{});
        ...
        std.debug.print("Encrypted '{s}' → '{s}' (key saved to key.bin)\n", .{input_path, output_path});
        ...
        // Read encrypted input (nonce || tag || ciphertext)
        const in_file = try fs.cwd().openFile(input_path, .{});
        defer in_file.close();
        ...
        var tag: [Aes256Gcm.tag_length]u8 = undefined;
        std.mem.copyForwards(u8, &tag, enc_data[Aes256Gcm.nonce_length..header_len]);
        ...
        std.debug.print("Decrypted '{s}' → '{s}' using key.bin\n", .{input_path, output_path});
