...
/// Simple BufWriter wrapper – we still stream, but no longer poke its
/// private buffer (that required unstable APIs). The scratch buffer used
/// during streaming is zero‑wiped, so key material does not linger.
...
impl<W: Write> Drop for ZeroizingWriter<W> {
    fn drop(&mut self) {
        // Best effort: flush to disk; scratch buffers are wiped elsewhere.
